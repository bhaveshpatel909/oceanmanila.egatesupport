...
class Employees extends CI_Controller {
	...
	//echo'<pre>';print_R($employees['data']);
	// foreach($employees['data'] as $emp){
		// $empppp=$emp['employee_id'];
...
//**calling card email functions//
 ...
 function callaing_card_mail(){
	...
	//print_r($ids);
	$result=$this->employees_actions->get_emp_emailpass($x);
	  ...
	  foreach($result as $key => $xyz)
		{
		    foreach($xyz as $key => $xy)
			{
				 ...
				 $from_email1 = "uplus.hrms@peza.com.ph"; 
                 $resemail=$xy->user_name;
				 
				       $password =$xy->user_password;
					 $password  = hash('sha512',  $password );
                 $subbb= "Your login details";
                 $msggg="Site Url : ".'http://uplushrms.peza.com.ph'."<br/>"."Username: ".$resemail."<br/>"."Password: ".' Please Reset Password To Access Your Account';
...
$new_name= $res['employee_no'].'_ID';
$pathinfo = pathinfo($imagePath);
$newpath='/ID_image/pic';
$ext = '.jpg';
//$newName  = $pathinfo['dirname'].'/copyavtar/'.$new_name;
$newName  ='/ID_image/pic/'.$new_name;
//print_r($newName);
...
$image_name=basename($imagePath);
$new_name= '/ID_image/sig/'.$res['employee_no'].'_SIG';
$pathinfo = pathinfo($imagePath);
$ext = '.jpg';
$newName  = $new_name;
//print_r($newName);
	...
	$this->load->model('employees_actions');
	 if ($this->input->post('submit')) {
            ...
            try {
                ...
                if($result){
echo"<script>alert('Imported successfully !'); window.location = 'http://uplushrms.peza.com.ph/request/processcallingcard';
</script>";
