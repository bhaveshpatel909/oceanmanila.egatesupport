...
<script>
    $('document').ready(function(){
        ...
        $('.assetbenefit_id').val(<?= $result?>);
        <?php }?>
        $("#employees_assetbenefits").html('<img src="images/ajax-loader.gif"/>');
        $.ajax({
            url:$('#employees_assetbenefits').attr('ajax_link'),
            success:function(html){
                $('#employees_assetbenefits').html(html);
            }
        })
