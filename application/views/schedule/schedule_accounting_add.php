<?php 
$html = ' Click <a href="dashboard">here</a> to back to schedule cash list. ';
if (is_numeric($result)){    
    $this->load->view('layout/success',array('message'=>$this->lang->line('Added.') . $html));?>
    <script>
        $('document').ready(function(){
			var  burl ="<?php echo base_url();?>";
			window.location.href=burl+"schedule/accounting_calendar";
...
<?php }else{
    ...
    <script>
        $('document').ready(function(){
			var  burl ="<?php echo base_url();?>";
			window.location.href=burl+"schedule/accounting_calendar";
//            row = current_table.fnAddData([<?= $result?>,$("#task_status_name").v
