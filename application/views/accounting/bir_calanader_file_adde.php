<?php if (is_numeric($result)){
    ...
    <script>
        $('document').ready(function(){
            row = current_table.fnAddData([$("#for_themonth").val(),$('#form_id option:selected').text(),null,$("#alertchk").val(),$("#amount").val(),null,null,null,'<a href="accounting/edit_bir_filee/<?= $result?>" data-target="#modal_window" data-toggle="modal" class="btn btn-outline btn-success"><i class="fa fa-edit"></i></a>']);
            oSettings = current_table.fnSettings();
            oSettings.aoData[row[0]].nTr.setAttribute('entity_id',<?= $result?>);
            $('#bir_file_id').val(<?= $result?>);
