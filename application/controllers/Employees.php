 ...
 * @property document_type_actions          $document_type_actions
...
class Employees extends CI_Controller {
    ...
    function index($page_id = 1) {
		...
		$emp_setting =$this->settings_actions->get_employee_settings();
	...
	function filteremp($page_id=1)
	{
		...
		$this->load->model('employee_memo');
		$empppp  = 	$this->employees_actions->get_employees_filter($page_id,$type,$is_active);
	
		$this->load->model('settings_actions');
		$emp_setting =$this->settings_actions->get_employee_settings();
	
        $this->load->view('employees/index', array(
            'employees' => $this->employees_actions->get_employees_filter($page_id,$type,$is_active),
			'enum_values' => $this->employee_memo->enum_select('employees','status'),
		//	'employees_active' => $this->employees_actions->get_employees_active(),
            'ienumber' => $this->employees_actions->total_inactivcount(),
            'taskscount' => $this->employees_actions->total_tasks($empppp),
	...
	function filterdepart($page_id=1)
	{
		//echo $page_id;
		 $type=$_GET['depart'];
		 $is_active=$_GET['is_active'];
		...
		$this->load->model('employee_memo');
		$empppp  = 	$this->employees_actions->get_employeesd_filter($page_id,$type,$is_active);
	
		$this->load->model('settings_actions');
		$emp_setting =$this->settings_actions->get_employee_settings();
	
        $this->load->view('employees/index', array(
            'employees' => $this->employees_actions->get_employeesd_filter($page_id,$type,$is_active),
			'enum_values' => $this->employee_memo->enum_select('employees','status'),
		//	'employees_active' => $this->employees_actions->get_employees_active(),
            'ienumber' => $this->employees_actions->total_inactivcount(),
            'taskscount' => $this->employees_actions->total_tasks($empppp),
	...
	function filtergroup($page_id=1)
	{
		//echo $page_id;
		 $type=$_GET['group_id'];
		 $is_active=$_GET['is_active'];
		...
		$this->load->model('employee_memo');
		$empppp  = 	$this->employees_actions->get_employeesg_filter($page_id,$type,$is_active);
	
		$this->load->model('settings_actions');
		$emp_setting =$this->settings_actions->get_employee_settings();
	
        $this->load->view('employees/index', array(
            'employees' => $this->employees_actions->get_employeesg_filter($page_id,$type,$is_active),
			'enum_values' => $this->employee_memo->enum_select('employees','status'),
		//	'employees_active' => $this->employees_actions->get_employees_active(),
            'ienumber' => $this->employees_actions->total_inactivcount(),
            'taskscount' => $this->employees_actions->total_tasks($empppp),
	...
	function filterstatus($page_id=1)
	{
		//echo $page_id;
		 $type=$_GET['status_id'];
		 $is_active=$_GET['is_active'];
		...
		$this->load->model('employee_memo');
		$empppp  = 	$this->employees_actions->get_employeesstatus_filter($page_id,$type,$is_active);
	
		$this->load->model('settings_actions');
		$emp_setting =$this->settings_actions->get_employee_settings();
	
        $this->load->view('employees/index', array(
            'employees' => $this->employees_actions->get_employeesstatus_filter($page_id,$type,$is_active),
			'enum_values' => $this->employee_memo->enum_select('employees','status'),
		//	'employees_active' => $this->employees_actions->get_employees_active(),
            'ienumber' => $this->employees_actions->total_inactivcount(),
            'taskscount' => $this->employees_actions->total_tasks($empppp),
	...
	function getbirthdaylist()
	{
		 ...
		 $this->load->view('employees/birthdaylist',array(
            'employees' => '',
          ));
	}
	function getbdaylisst_report()
	{
      ...
      $this->load->model('reports_actions');
		...
		$this->load->view('employees/birthdaygetlist',array(
            'employees' => $this->employees_actions->get_birthdayemployees(),
			'postdata' => json_encode($postData),
          ));
		
	}
	function printbdaylisst_report()
	{
		$this->load->model('settings_actions');
		...
		$this->load->model('employees_actions');
        $data = $this->employees_actions->print_birthdayemployees($start_date,$end_date);
		$html = $this->load->view('employees/print_birthday_list', array('data' => $data,'logo' => $logo, 'from' => $start_date, 'to' => $end_date), TRUE);
        //exit($html);
   ...
   function inactive($page_id = 1) {
		...
		{
			$count=  $this->employees_actions->get_employeesss($page_id, FALSE);
			$ar=array();
			...
			$len=count($ar);
			 ...
			 {
				$len= $this->employees_actions->total_inactivcount();
			 }
    ...
    function edit_employee($employee_id = 0) {
        ...
        $this->load->view('employees/employee_edit', array(
            ...
            'conimage' => $this->employees_actions->get_contract($avatar),
			...
			'approve_record'=>$this->timeoff_actions->get_records_approved($status=array('approved'),$employee_id),
			'denied_record'=>$this->timeoff_actions->get_records_denied($status=array('denied'),$employee_id),
        ));
    }
	...
	function update_status(){
		...
		$this->employees_actions->update_status($emp_id,$status);
	}
    ...
    function new_employe() {
        $this->load->view('employees/employee_new',array(
		'departments' => $this->departments_actions->get_departments(),
'employee_memor' => $this->employee_memo->get_employee_memo_list($employee_id)));
    }
...
function delete_file_from_folder()
{
	...
	if($deletefiles){
		// echo "<pre>";
		// print_r($_SERVER);
 $directory = $_SERVER['DOCUMENT_ROOT'] . '/files/ID_image/pic/';
...
//die("here");
	...
	{
		if(unlink($path)){
		   $this->db->update('employees', array('copy_avatar' => '')) ;
 }
...
//die("here");
	...
	redirect($this->config->item('base_url').'request/processcallingcard');
}
...
function delete_sigfile_from_folder(){
 ...
 $directory = $_SERVER['DOCUMENT_ROOT'] . '/files/ID_image/sig/';
    $filesd = glob($directory . '*');
	foreach($filesd as $path)
	{
		if(unlink($path)){
...
function multiple_sign_upload(){
 ...
 redirect('/request/processcallingcard');	
 }

 function work_menual_uploade(){
 ...
 function save_position() {
	...
	function save_group() {
        $this->load->model('employees_actions');
        $this->employees_actions->save_group();
...
//    }
    ...
    function edit_license($item_id = 0) {
        ...
        $this->load->model('attachments_actions');
        $this->load->helper('fa-extension');
        $list_document_type = $this->document_type_actions->get_201_document_type();
    ...
    function save_license() {
        ...
        $this->form_validation->set_rules(array(
            ...
            array('field' => 'employee_id', 'rules' => 'required', 'label' => 'employee_id'),
            array('field' => 'document_type', 'rules' => 'required', 'label' => $this->lang->line('201 File Document Type')),
        ));
    ...
    function new_license($employee_id = 0) {
        $list_document_type = $this->document_type_actions->get_201_document_type();
        $this->load->view('employees/license_new', array('employee_id' => $employee_id, 'list_document_type' => $list_document_type));
    }
    ...
    function download_attachment($attachment_id = 0) {
       ...
       // $this->attachments_actions->download_attachment($attachment_id);
        ...
        $filewithpath = 'http://uplushrms.peza.com.ph/files/attachments/'. $attachment['file'];
      //  echo BASEPATH.'../files/attachments/'.$attachment['file'];
        //echo $filewithpath; 

        $filewithpath = 'home1/pezacomph/public_html/uplushrms/files/attachments/minebea_august.JPG';
...
//**calling card email functions//
	  ...
	  foreach($result as $key => $xyz)
		{
		    foreach($xyz as $key => $xy)
			{
                 ...
                 $msggg="Site Url : ".'http://uplushrms.peza.com.ph'."<br/>"."Username: ".$resemail."<br/>"."Password: ".' Please Reset Password To Access Your Account<br><p style="color:red;font-weight:bolder;">Your Other Details See Below  Cheack With  Carefully If Any Mistakes or  Any Changes Please Contact to Admin </p>'.
				 "Name : ".$name."<br/>".
 ...
 function save_image_desktop(){
	 ...
	 $this->load->helper('download');
	$file = $this->config->item('base_url').'files/ID_image/pic/';
$local_file = 'ID_image';
...
//            array('field'=>'description','rules'=>'required','label'=>$this->lang->line('Description'))
    ...
    function print_contract($contract_id = 0) {
		...
		//$html = $this->load->view('employees/contract_print', array('contract' => $contract, 'images' => $images), TRUE);
        ini_set('memory_limit', '32M'); // boost the memory limit if it's low <img src="https://s.w.org/images/core/emoji/72x72/1f609.png" alt="😉" draggable="false" class="emoji">
        //this the the PDF filename that user will get to download
...
//**** print_leave_request
 function print_employee_leave($record_id = 0) {
        $this->load->model('employees_actions');
		$this->load->model('timeoff_actions');
        $this->load->model('settings_actions');
        $logo = $this->settings_actions->get_setting('company_logo');
        ...
        $employee = $this->employees_actions->print_employee_leave($record_id);
		//$family = $this->employees_actions->get_family($employee_id);

        $data = array(
            'company' => array('logo' => $logo, 'name' => $company_name, 'address' => $company_address, 'phone' => $company_phone),
            'family' => $emppdata, 'employee' => $employee
        );
        //PDF generating
        $html = $this->load->view('timeoff/employee_leave_print', $data, TRUE);
        ini_set('memory_limit', '32M'); // boost the memory limit if it's low <img src="https://s.w.org/images/core/emoji/72x72/1f609.png" alt="😉" draggable="false" class="emoji">
        //this the the PDF filename that user will get to download
        $pdfFilePath = str_replace(" ", "_", $employee['fullname']) . "_employee.pdf";
...
$new_name= end(explode('/',$res['employee_no']));
...
function outputcsv() {
	...
	$this->load->dbutil();
      ...
      $query = "SELECT employees.employee_id,users.is_active, employees.is_regular,employees.name, employees.nick_name,employees.contactno,employees.employee_no,employees.email ,employees.avatar, employees.sign, employees.copy_avatar,employees.copy_sign,employees.tin,employees.ssn,
	   employees.Pag_Ibig_No, 
employees.healthno,employees.birth_date,employees.gender,employees.address,departments.department_name,employees.hired_date
FROM employees
  ...
  // die("here");
	...
	$this->load->model('employees_actions');
	 $this->load->dbutil();
        ...
        $delimiter = ",";
        $newline = "\r\n";
        $filename = "Employee Master List Expiry.csv";
		 $compare = date('Y-m-d');
		//$export =$this->employees_actions->export_data();
       $query = "SELECT employees.employee_id,employees.is_regular,users.is_active, employees.name, employees.nick_name,employees.contactno,employees.employee_no,employees.email ,employees.avatar, employees.sign, employees.copy_avatar,employees.copy_sign,employees.tin,employees.ssn,
	   employees.Pag_Ibig_No, 
employees.healthno,employees.birth_date,employees.gender,employees.address,departments.department_name,employees.hired_date,employees_contract.contract_expiry
FROM employees
INNER JOIN departments ON employees.department_id = departments.department_id 
INNER JOIN users ON employees.employee_id = users.employee_id
INNER JOIN employees_contract ON employees_contract.employee_id = users.employee_id
   WHERE users.is_active='1' AND employees_contract.contract_expiry>".$compare;
...
// die("here");				 
                ...
                $result1 = $this->employees_actions->importuser_data($data_user); 
                
				 
                if($result1){
					//echo $result;
echo"<script>alert('Imported successfully !'); window.location = 'https://kiwa.egatesupport.com/request/processcallingcard';
</script>";
				ECHO "SUCCESS";	
   ...
   // foreach($export as $row){
    // print_r($row);
	 // fputcsv($output,$row);  
      // }  
      // fclose($output);  
	  function print_contract_type_list()
	  {
		   $this->load->model('settings_actions');
        $logo = $this->settings_actions->get_setting('company_logo');
         $this->load->model('contract_type');
       // $this->load->view('employees/contract_type_list', array('contract_types' => $this->contract_type->get_contract_type_list()));
        //_custom_debug($data);
        //PDF generating
        $html = $this->load->view('employees/contract_template_print', array('logo' => $logo, 'contract_template' => $this->contract_type->get_contract_type_list()), TRUE);
        ini_set('memory_limit', '32M'); // boost the memory limit if it's low <img src="https://s.w.org/images/core/emoji/72x72/1f609.png" alt="😉" draggable="false" class="emoji">
        //this the the PDF filename that user will get to download
        $pdfFilePath = "Contract_Templates.pdf";
 ...
 function document_expiration() {
        $this->load->model('employees_actions');
        $this->load->helper('fa-extension');
		$list_document_type = $this->document_type_actions->get_201_document_type();
        $this->load->view('document_expiration/index', array(
            'licenses' => $this->employees_actions->get_all_licenses(),
            'list_document_type' => $list_document_type,
        ));
    }
	function exportdocumentexp($start_date="0", $end_date="0",$expiration_date="0",$document_type="0",$expired_filter="0")
	{
        ...
        $delimiter = ",";
        $newline = "\r\n";
        $filename = "Document Expiration List .csv";
		 $compare = date('Y-m-d');
		//$export =$this->employees_actions->export_data();
        $days_to_expire = "TIMESTAMPDIFF(day,license_expiry,NOW()) * -1";
        $this->db->select('employees_licenses.license_id,employees_licenses.employee_id, 201_document_type.document_type_name as license_name,'.$days_to_expire.' as days_to_expire,employees_licenses.license_expiry,days_to_alert,IF('.$days_to_expire.' > 0, '.$days_to_expire.' - days_to_alert, "EXPIRED") as remaining_days,employees_licenses.license_number');
        $this->db->join('users', 'employees_licenses.employee_id = users.employee_id', 'left');
        $this->db->join('201_document_type', '201_document_type.document_type_id = employees_licenses.document_type_id', 'left');
        $this->db->where('users.is_active',1);
        if($start_date != "0" && $end_date != "0"){
            $this->db->where('license_expiry >= "'.$start_date.'" and license_expiry <= "'.$end_date.'"');
        }
        if($expiration_date != "0"){
            $this->db->where('DATEDIFF(employees_licenses.license_expiry , NOW()) <= '.$expiration_date);
        }
        if($document_type != "0"){
            $this->db->where('employees_licenses.document_type_id',$document_type);
        }
        if($expired_filter != "0"){
            if($expired_filter == "1"){
                $this->db->where('DATEDIFF(employees_licenses.license_expiry , NOW()) <= 0');
            }else{
                $this->db->where('DATEDIFF(employees_licenses.license_expiry , NOW()) > 0');
            }
        }
        $result = $this->db->get('employees_licenses');
