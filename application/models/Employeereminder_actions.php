 ...
 */
class Employeereminder_actions extends Base_model {

    function get_employeereminder($page_id = 1) {
        $this->db
                ->select('SQL_CALC_FOUND_ROWS *,reminderlist.reminder_name as cname', FALSE)
				->join('reminderlist', 'reminderlist.reminder_id = employeereminder.employeereminder_category_id', 'LEFT')
                ->limit(100, ($page_id - 1) * 100)
               // ->order_by('employeereminder.uploaded', 'DESC')
		...
		foreach ($result['data'] as $key => $document) {
            $attachments = $this->db->select('*')->where(array('object'=> $document['employeereminder_id'],'type'=> 'employeereminder'))
...
//                ->limit(12, ($page_id - 1) * 12)
    ...
    function get_document($document_id, $with_permissions = TRUE) {
        $document = $this->db
                ->select('*')
                ->where('employeereminder.employeereminder_id', $document_id)
        ...
        $permissions = $this->db
                ->select('type,value')
                ->where('employeereminder_id', $document_id)
                ->get('employeereminder_permissions')
                ->result_array();
	 ...
	 function get_documentrow($document_id) {
        $document = $this->db
                ->select('*')
                ->where('employeereminder.employeereminder_id', $document_id)
                ->get('employeereminder')
...
// echo'</pre>';
	...
	//////////////////////////////////////////////////////////Mail document start///////////////////////////////////////////////
		...
		function mailget_document($document_id) {
   $document = $this->db
                ->select('*')
                ->where('employeereminder.employeereminder_id', $document_id)
                ->get('employeereminder')
   ...
   $documentgl = $this->db
       ...
       return $documentgl;
	    ...
	    function mailget_attachments($document_id)
      {
          return $this->db
                      ->select('*')
                      ->where(array('type'=>'workmanual','object'=>$document_id))
                      ->get('attachments')
                      ->result_array();
      }
    ...
    function save_document() {
        $document = array(
            ...
            'employeereminder_category_id' => $this->input->post('employeereminder_category_id'),
        ...
        if ($this->input->post('document_id') == '0') {
            $this->db->insert('employeereminder', $document);
            $result = $document_id = $this->db->insert_id();

            $this->save_permissions($document_id);
        } else {
            $this->db->update('employeereminder', $document, array('employeereminder_id' => $this->input->post('document_id')));
            $result = TRUE;
            $document_id = $this->input->post('document_id');
        }

        $this->load->model('attachments_actions');
        if (!$files = $this->attachments_actions->upload_attachments_not_birg('employeereminder', $document_id)) {
            return FALSE;
        }
...
//    }
    ...
    private function save_permissions($document_id) {
        $this->db->delete('employeereminder_permissions', array('employeereminder_id' => $document_id));

        if ($this->input->post('permissions_all')) {
            $this->db->insert('employeereminder_permissions', array(
                'employeereminder_id' => $document_id,
                'type' => 'all',
                'value' => '*'
            ));
        ...
        if ($this->input->post('employees_list') AND count($this->input->post('employees_list')) > 0) {
            ...
            $this->db
                    ->query('INSERT INTO employeereminder_permissions(employeereminder_id,type,value)
    ...
    function delete_document($document_id) {
        ...
        $this->db->delete('employeereminder', array('employeereminder_id' => $document_id));
        $this->db->delete('employeereminder_permissions', array('employeereminder_id' => $document_id));
    ...
    function get_employee_documents($page_id = 1) {
        ...
        $this->db
                ->select('SQL_CALC_FOUND_ROWS employeereminder.document_id,file,extenstion, description', FALSE)
                ->join('employeereminder', 'employeereminder.employeereminder_id = employeereminder_permissions.employeereminder_id', 'LEFT')
                ->limit(12, ($page_id - 1) * 12)
                ->order_by('file');
	...
	function get_employeereminder_department()
