 ...
 */
class Workmanual_actions extends Base_model {

    function get_workmanual($page_id = 1) {
        $this->db
...
//                ->limit(12, ($page_id - 1) * 12)
    ...
    function get_document($document_id, $with_permissions = TRUE) {
        $document = $this->db
                ->select('*')
                ->where('workmanual.workmanual_id', $document_id)
                ->get('workmanual')
                ->row_array();
        ...
        $permissions = $this->db
                ->select('type,value')
                ->where('workmanual_id', $document_id)
   ...
   $documentg = $this->db
	 ...
	 function allemp_documentt($emp_id) {
   $documentg = $this->db
                ->select('*')
   ...
   $documentgl = $this->db
    ...
    function save_document() {
        $document = array(
            ...
            'workmanual_category_id' => $this->input->post('workmanual_category_id'),
        ...
        if ($this->input->post('document_id') == '0') {
            $this->db->insert('workmanual', $document);
            $result = $document_id = $this->db->insert_id();

            $this->save_permissions($document_id);
        } else {
            $this->db->update('workmanual', $document, array('workmanual_id' => $this->input->post('document_id')));
            $result = TRUE;
            $document_id = $this->input->post('document_id');
        }

        $this->load->model('attachments_actions');
        if (!$files = $this->attachments_actions->upload_attachments_not_birg('workmanual', $document_id)) {
            return FALSE;
        }
...
//    }
    ...
    private function save_permissions($document_id) {
        ...
        if ($this->input->post('permissions_all')) {
            $this->db->insert('workmanual_permissions', array(
                'workmanual_id' => $document_id,
                'type' => 'all',
                'value' => '*'
            ));
        ...
        if ($this->input->post('employees_list') AND count($this->input->post('employees_list')) > 0) {
            ...
            $this->db
                    ->query('INSERT INTO workmanual_permissions(workmanual_id,type,value)
    ...
    function delete_document($document_id) {
        ...
        $this->db->delete('workmanual', array('workmanual_id' => $document_id));
        $this->db->delete('workmanual_permissions', array('workmanual_id' => $document_id));
	...
	function get_workmanual_department()
