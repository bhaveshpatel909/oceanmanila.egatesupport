<?php $this->load->view('layout/header',array('title'=>$this->lang->line('BIR file ( Uplus )'),'forms'=>TRUE,'tables'=>TRUE,'icheck'=>TRUE)) ?>
<script>
...
.sustmclas td:nth-child(3) {    text-align: left;}
.sustmclas td:nth-child(5) {    text-align: left;}
.sustmclas td:nth-child(7) {    text-align: left;}

</style>
<div id="wrapper">
    <?php $this->load->view('layout/menu',array('active_menu'=>'bir_calender_files'))?>
    <div id="page-wrapper" class="gray-bg">
        <?php $this->load->view('layout/page_header')?>
        
        <div class="row wrapper border-bottom white-bg page-heading">
            <div class="col-lg-8">
                <h2><?= $this->lang->line('BIR file ( Uplus )')?></h2>
                <ol class="breadcrumb">
                    <li>
            ...
            <div class="col-lg-4">
                <div class="title-action">
                    <a href="accounting/new_bir_calender_file" class="btn btn-primary" data-target="#modal_window" data-toggle="modal">
                            ...
                            <div id="save_result"></div>
                                <table class="sustmclas table table-striped table-bordered table-hover data_table" id="table">
                                    ...
                                    <tbody style="text-align: center;">
                                        ...
                                        <tr entity_id="<?= $bir_file['bir_c_file_id']?>" class="eachro">
                                            <td><?= $bir_file['for_themonth']?></td>
                                            <td class="col2"><?= $bir_file['form_name']?></td>
										<?php
										
										
											$bir_file_id = $bir_file['bir_c_file_id'];
											$type = 'bir_calander_file';
											$this->load->model('accounting_actions');
											$bir_attach = $this->accounting_actions->get_bir_file_attach($bir_file_id, $type);
											// echo "<pre>";
											// print_R($bir_attach);
										?>	
                                            <td><a href="<?= $base_url ?>/files/attachments/<?= $bir_attach['location']?>" target="_blank"><?php echo $bir_attach['file'] ?></a></td>
											<?php 
										...
										?>	
											...
											?>
											<td class="xyz">
											<input type="checkbox" style="width: 109px; height: 21px;" <? if($bir_file['alertchk']== 1){ ?> checked <?php } ?> name="alertchk" id="mySelectorr"  value="<? echo 'check='. $bir_file['alertchk'];echo '&formid='.$bir_file['bir_c_file_id'];
                                            ...
                                            <td>
                                                <a class="btn btn-outline btn-success" href="accounting/edit_bir_calnder_file/<?= $bir_file['bir_c_file_id']?>" data-target="#modal_window" data-toggle="modal">
