...
<script>
    $('document').ready(function(){
        ...
        $('.performance_id').val(<?= $result?>);
        <?php }?>
        $("#employees_performances").html('<img src="images/ajax-loader.gif"/>');
        $.ajax({
            url:$('#employees_performances').attr('ajax_link'),
            success:function(html){
                $('#employees_performances').html(html);
            }
        })
