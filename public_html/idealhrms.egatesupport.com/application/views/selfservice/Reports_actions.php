...
class Reports_actions extends Base_model {
    ...
    function clock_default() {
        ...
        if (isset($_POST['employee']) && $_POST['employee'][0]!="") {
			$result = $this->db
                        ->select('remarks,overtime_remark,record_id,start_time,end_time,penality_time, DATE_FORMAT(start_time,"%Y%m%d") as date_id, comments,time_out')
...
//        _custom_debug($result);
	...
	function get_allCredit_byorder($odr)
    {
		return $this->db
			...
			->select_sum('Amount')
			->where(array('Order_No' => $odr, 'Transaction_Type' => 'Orders-Item Charges-Item Price Credit'))
			->get('excell')
			->result_array();
	}
	...
	/*******************************************Reports of clock model  start*****************************************************/
	 ...
	 function clock_ordefault() {
        if (isset($_POST['employee'])) {
            $this->db->where('orpunch_clock.employee_id', (int) $_POST['employee'][0]);
        } else {
            $this->db
                    ...
                    ->join('employees_positions', 'employees_positions.employee_id = orpunch_clock.employee_id AND employees_positions.is_current=1', 'LEFT')
                    ->join('positions', 'positions.position_id = employees_positions.position_id', 'LEFT')
                    ->join('departments', 'departments.department_id = employees.department_id', 'LEFT');
        }
