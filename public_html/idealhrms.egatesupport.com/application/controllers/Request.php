...
class Request extends CI_Controller {
	...
	function processcallingcardnew($page_id = 1) {
	...
	$pin_employee =$_GET['checkval'];
        ...
        $this->load->view('employees/process_callingcardnew', array(
	...
	function p_calling_cards()
	{
		
		
		$this->load->library('datatables');
		
        $this->datatables->select('employees.employee_id, employees.name, employees.avatar,employees.sign, employee_list.status,employees.employee_no,employees.nick_name,employees.ssn,employees.copy_avatar,employees.copy_sign,employees.tin,
				employees.Pag_Ibig_No,employees.employee_status_note ,employees.birth_date,employees.hired_date,employees.employee_relation,employees.employee_contactperson,employees.confirm_date_pp,employees.employee_address, ,employees.contactno,employees.email,employees.pin,employees.copy_image_path,employees.copy_sig_path,employees.healthno,positions.position_name,users.is_active,departments.department_name,grouplist.group_name,users.user_ip as user_ip,employees.is_regular');
		$this->datatables->from('employees');
		if(isset($_GET['s_id']))
		{
			$srch =$_GET['s_id'];
			$this->datatables->like('employees.name', $srch);
           // $this->datatables->or_like('positions.position_name', $srch);
           // $this->datatables->or_like('departments.department_name', $srch);
		}
		$this->datatables->join('positions', 'positions.position_id = employees.position_id', 'LEFT');
        $this->datatables->join('departments', 'departments.department_id = employees.department_id', 'LEFT');
        $this->datatables->join('grouplist', 'grouplist.group_id = employees.group_id', 'LEFT');
        $this->datatables->join('employee_list', 'employee_list.id = employees.status', 'LEFT');
        $this->datatables->join('users', 'users.employee_id = employees.employee_id', 'LEFT');
      //  $this->datatables->join('employees_contract', 'employees_contract.employee_id = employees.employee_id', 'LEFT');
		if(isset($_GET['is_active']) && $_GET['is_active']==0)
		{
			$this->datatables->where('users.is_active=', 0);
		}
		else
		{
		$this->datatables->where('users.is_active=', 1);
		}
		if(isset($_GET['d_id']) && $_GET['d_id']!="")
		{
			$did =$_GET['d_id'];
			$this->datatables->where('departments.department_id=', $did);
		}
		if(isset($_GET['checkval']) && $_GET['checkval']!="")
		{
			$checkval =$_GET['checkval'];
			$this->datatables->where('employees.pin =', '1');
		}
		if(isset($_GET['g_id']) && $_GET['g_id']!="")
		{
			$gid =$_GET['g_id'];
			$this->datatables->where('grouplist.group_id=', $gid);
		}
		if(isset($_GET['hs_id']) && $_GET['hs_id']!="")
		{
			$hs_id =$_GET['hs_id'];
			$this->datatables->where('employee_list.id=', $hs_id);
		}
		// $today = new DateTime();
     $compare = date('Y-m-d');
		
		$this->datatables->where('users.user_name!=', 'k09178548787@gmail.com');
		//$this->datatables->where('employees_contract.contract_expiry>=', $compare);
		
		
		//$this->datatables->group_by('employees.employee_id');
		echo  $this->datatables->generate();
		//echo $this->db->last_query();
	}
	function emp_cont_reminer()
	{
		 $this->load->model('employees_actions');
		$employee= $this->employees_actions->get_employee($_POST['eid']);
...
//    }
	...
	function delete_avtar(){
		...
		$emmp=$_GET['employee'];
		$this->drequest_actions->del_avtar($emmp);
		//echo'Delet Avtar';
	}
