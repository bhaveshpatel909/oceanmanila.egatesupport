...
$(document).ready(function() {
    $('#employee_tab').click(function(e) {  
      window.location.href = "http://uplushrms.peza.com.ph/employees";
    });
});
        ...
        <ul class="nav" id="side-menu">
			...
			<li  <?= ($active_menu == 'orignalclock') ? 'class="active"' : '' ?>  >
                            ...
                            <a href="dashboard/orignaldashboard">
							 <i class="fa fa-th-large"></i>
							 <span class="nav-label"><?php echo "Original Punch clock "; ?></span> 
            ...
            <?php if ($this->user_actions->is_allowed(array('admin', 'employees', 'evaluations', 'discipline','reminder','request','performance'))) { ?>
                <li <?= (in_array($active_menu, array('employees','inactive_employees', 'employees_new', 'contract_history', 'discipline', 'discipline_new', 'evaluations', 'evaluation_new', 'evaluation_edit','employee_reminder','reminder','request','employee_request','performance','evaluation'))) ? 'class="active"' : '' ?>>
                    <a href="employees">
                    ...
                    <ul class="nav nav-second-level circle1">
                        <?php if ($this->user_actions->is_allowed('employees')) { ?>
                            <li <?= (in_array($active_menu, array('employees', 'employees_new', 'contract_history','inactive_employees','employee_reminder','request'))) ? 'class="active"' : '' ?>>
                                <a href="employees">
            ...
            <?php if ($this->user_actions->is_allowed(array('accounting','bir_files','petty_cash','lazada','schedule_bir_calendar' ,'bir_calender_files','bir_egate_files','bir_modory_files','monthly_bill_payment','document_category_id'))) { ?>
                <li <?= (in_array($active_menu, array('accounting','bir_files','petty_cash','schedule_bir_calendar','bir_calender_files','bir_egate_files','bir_modory_files','employee_Lazada','lazada','check_writer','monthly_bill_payment','document_category_id'))) ? 'class="active"' : '' ?>>
                    <a href="accounting">
 ...
 // [2] => Array
   ...
   <?php //} ?>
            ...
            <?php if (1 == 1) { ?>
                ...
                $tasks_number = get_tasks(); 

print_r($tasks_number);
                ...
                <li <?= (in_array($active_menu, array('tasks', 'attention_updated_tasks', 'regular_tasks', 'all_tasks', 'assigned_tasks', 'unassigned_tasks', 'completed_tasks','pending_task', 'attention_tasks'))) ? 'class="active"' : '' ?>>
                    ...
                    <ul class="nav nav-second-level">
						...
						<li <?= ((in_array($active_menu, array('pending_task'))) ? 'class="active"' : '') ?>>
                            <a href="tasks/pending_task"><?= $this->lang->line('Pending Task') ?>
                            <span class="text-warning pull-right"><?=$tasks_number['pending_task']?></span>
							<?php
								$query ="SELECT COUNT(process) FROM `tasks` WHERE process<100";
                        ...
                        <li <?= ((in_array($active_menu, array('assigned_tasks'))) ? 'class="active"' : '') ?>>
                            <a href="tasks/index/assigned"><?= $this->lang->line('Assigned & Accepted') ?>
                            <span class="text-warning pull-right"><?=$tasks_number['assigned']?></span>
                            </a>
                        </li>
                        <li <?= ((in_array($active_menu, array('attention_tasks'))) ? 'class="active"' : '') ?>>
                            <a href="tasks/attention_tasks"><?= $this->lang->line('Questioned Task') ?>
                        ...
                        <li <?= ((in_array($active_menu, array('attention_updated_tasks'))) ? 'class="active"' : '') ?>>
                            <a href="tasks/attention_updated_tasks"><?= $this->lang->line('Answered Task') ?>
 ...
 // [2] => Array
        ...
        // )
            ...
            <?php if ($this->user_actions->is_allowed('leave_tracking')) { ?>
            ...
            <?php if ($this->user_actions->is_allowed('reports')) { ?>
                <li <?= (in_array($active_menu, array('work_evaluation_rules', 'reports_skills', 'reports_clock','assets_report','orignalreports_clock', 'report_discipline', 'employee_evaluation', 'company_rules'))) ? 'class="active"' : '' ?>>
                    <a href="reports">
            ...
            <?php if ($this->user_actions->is_allowed('admin')) { ?>
                <li <?= (in_array($active_menu, array('document_category','requestt', 'setting_letter', 'evaluation_templates', 'customer_item', 'schedule_item', 'company_settings', 'company_email', 'company_positions', 'resign_reasons', 'departments', 'processing_errors', 'unsent_emails', 'discipline_actions', 'discipline_reasons', 'discipline_category', 'task_status', 'task_category', 'task_type','evaluation_list', 'evaluation_category', 'contract_type','bank_list','bir_forms','petty_cash_item','monthly_bill_list', 'employee_memo', 'reminder_list'))) ? 'class="active"' : ''; ?>>
                    ...
                    <ul class="nav nav-second-level">
                        ...
                        <li <?= (in_array($active_menu, array('contract_type', 'company_positions', 'departments', 'employee_memo', 'reminder_list'))) ? 'class="active"' : '' ?>>
                            <a href="discipline">
