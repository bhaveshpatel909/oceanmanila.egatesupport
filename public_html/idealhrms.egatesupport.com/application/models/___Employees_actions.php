...
class Employees_actions extends Base_model {
    ...
    function get_employees($page_id,$is_active = TRUE) {
        ...
        $this->db
                ->select('SQL_CALC_FOUND_ROWS employees.employee_id, employees.name, employees.employee_no, employees.avatar, employees.status,employees.late_Time,employees.petteycashliquidate, employees.employee_status_note , positions.position_name,users.is_active,departments.department_name', FALSE)
	 ...
	 function get_employeesss($page_id,$is_active = TRUE) {
        ...
        $this->db
                ->select('SQL_CALC_FOUND_ROWS employees.employee_id, employees.name, employees.avatar, employees.status, employees.employee_status_note , positions.position_name,users.is_active,departments.department_name,users.user_ip as user_ip', FALSE)
                //->join('employees_positions','employees_positions.employee_id = employees.employee_id AND is_current=1','LEFT')
                ->join('positions', 'positions.position_id = employees.position_id', 'LEFT')
                ->join('departments', 'departments.department_id = employees.department_id', 'LEFT')
                ->join('users', 'users.employee_id = employees.employee_id', 'LEFT')
	...
	function get_employeesssf($id ,$page_id,$is_active = TRUE) {
        ...
        $this->db
                ->select('SQL_CALC_FOUND_ROWS employees.employee_id, employees.name, employees.avatar, employees.status, employees.employee_status_note , positions.position_name,users.is_active,departments.department_name', FALSE)
                //->join('employees_positions','employees_positions.employee_id = employees.employee_id AND is_current=1','LEFT')
                ->join('positions', 'positions.position_id = employees.position_id', 'LEFT')
                ->join('departments', 'departments.department_id = employees.department_id', 'LEFT')
                ->join('users', 'users.employee_id = employees.employee_id', 'LEFT')
...
//         die($this->db->last_query());
	...
	$empid =explode(",", $emp_id);
	
	
	
	foreach($empid as $emp){
		
	$this->db->where (array('employee_id' =>$emp));
			$q = $this->db->get('users');
