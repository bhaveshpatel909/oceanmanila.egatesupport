...
function submit_form1(form_selector, target)
{
    target = target || "#save_result";
...
function init_icheck()
{
    ...
    $('.checkItem').on('ifChanged', function (event) {
        var checkLength = $('.checkItem:checked').length;
        var total = $('.checkItem').length;
        //alert(event.type + ' total ' + total + ' checked ' + checkLength);
        if (checkLength == 0 || checkLength != total) {
            $('#selectall').iCheck('uncheck');
        } else if (checkLength == total) {
...
function ajax_query(link, callback)
{
    callback = callback || "";
...
function preview_record(discipline_id) {
